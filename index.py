...
@app.route('/presente_iregular_backward', methods=['POST'])
def presente_iregular_backward():
    resp = make_response('presente_iregular_backside')
    ...
    const = request.cookies.get('const')
    try:
        presente_iregular_prog = int(request.cookies.get("presente_iregular_prog"))
    except:
        presente_iregular_prog = 0

    q = 0
    for i in presente_iregulares:
        if i[0] == verb:
            right_ends = i[1:]
            break
        else:q+=1
